...
for i = 2*T+1 :-1:1
    ...
    if i == 2*T+1
        for j=1:nnz(list)
            ...
            M_star_FirstPass.Nodes.Decision_Value(list(j)) =  bwarea(M_star_SecondPass.Nodes.Agent_Region{list(j)}) - Negtive_Reward* M_star_SecondPass.Nodes.Agent_Detection_time(list(j)) - Negtive_Teammate*(M_star_SecondPass.Nodes.Teammate_Detection_time(list(j)));
...
for k =1:2*T+1
    if mod(k,2)
        Agent_path_x((k+1)/2) = M_star_FirstPass.Nodes.Agent_x(M_star_FirstPass_Node_path(k));
        Agent_path_y((k+1)/2) = M_star_FirstPass.Nodes.Agent_y(M_star_FirstPass_Node_path(k));
    else
        Opponent_path_x(k/2) = M_star_FirstPass.Nodes.Opponent_x(M_star_FirstPass_Node_path(k));
        Opponent_path_y(k/2) = M_star_FirstPass.Nodes.Opponent_y(M_star_FirstPass_Node_path(k));
    end
end
Initial_Agent = [Agent_path_x(2);Agent_path_y(2)];
Initial_Opponent = [Opponent_path_x(2);Opponent_path_y(2)];
Initial_Agent_Region = M_star_FirstPass.Nodes.Agent_Region{ThirdPass_Node_path(3)};
WiseUp = M_star_FirstPass.Nodes.WiseUp(ThirdPass_Node_path(3));